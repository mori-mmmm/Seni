...
class Bot(discord.Client):
    ...
    async def improve_bot(self, request, channel):
        ...
        llm_res = await get_llm_ret(prompt)
        res = llm_res[0]
        prompt = f"""{res}\n위의 텍스트에서 코드에 해당하는 부분만 추출해줘. 만약 코드가 ```와 같이 코드 블록으로 감싸져 있으면 내용물만 빼내서 출력해줘. 부가적인 코드에 대한 설명이 붙어있다면 제거하고, 그 자체로 실행가능한 형태로 코드 영역만 추출해줘. 코드블록으로 감싸지도 말고 아무 추가적인 말도 덧붙이지 말아줘."""
        llm_res = await get_llm_ret(prompt)
        res = llm_res[0]
